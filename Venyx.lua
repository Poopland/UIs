...
local InputConnection = {
    unpack(getconnections(input.InputBegan)),
    unpack(getconnections(input.InputEnded))
}

for i,v in pairs(InputConnection) do
    if checkcaller() then
        if v:Fire("CheckIsVenyx") == "Connection from Venyx Megumint Edition" then
           v:Disable() 
        end
    end
end
...
do
	...
	function utility:InitializeKeybind()
		...
		input.InputBegan:Connect(function(key)
            if key == "CheckIsVenyx" then
                return "Connection from Venyx Megumint Edition"
            end
...
do
	...
	function library.new(title,id)
        if game.CoreGui:FindFirstChild(title..id) then
